...
Put $H(n)=F(n)^{2/3}$.  Prove that, for some $\delta>0$ and all $m,n$,
\[
 H(m+n)\le H(m)+H(n)+O\bigl((m+n)^{1-\delta}\bigr),
\]
or establish an equivalent recurrence whose normalized errors are summable
on geometric scales.  The standard almost-subadditive lemma would then make
$H(n)/n$ converge, and hence make $L_n=(H(n)/n)^{3/2}$ converge.  External
...
is power-saving composition for $F(n)^{2/3}$ or an equivalent summable
finite-temperature recurrence.  No candidate pair of separated infinite
