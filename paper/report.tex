...
when such compression is impossible.
\end{abstract}
...
There is a still coarser two-replica sufficient condition.  Put
...
\end{equation}
forces a nontrivial character satisfying \eqref{eq:one-character}.  The
quantity $U_g$ counts pairs of child states that violate the target under the
same switching, so \eqref{eq:two-replica-quotient} replaces all stratified
shells by one exact overlap statistic.
...
is finite evidence about the hierarchy, not an asymptotic shell theorem.  The
two-replica condition \eqref{eq:two-replica-quotient} already certifies the
displayed one-character gain at $3+3$ and $4+4$, but not at $3+4$ or $3+5$.
...
the wrong target for that route.
\end{problem}
...
one-character gains, and the quotient-dimension counting obstruction.
...
bound rules out compression of isolated gains.  No candidate pair of separated infinite
